...
## 攻坚顺序（每步 device 可验证）
1. **run#3 已跑（2026-07-11）—— 过所有 arg 契约，剩 StackedDeviceTensor `[r,k]` 结构 gate**：
   real-weight IPC harness 走到 rt.run（8 exporter 池 + `import_ipc_all` 8 peer_bases + compile OK +
   42 args + 8 chip ready），报 `ValueError: StackedDeviceTensor only supports whole-shard slicing on
   the leading dim; got partial slice 0 on axis 1 (shard shape (12,4096,1024))`。根因：N=1 host_orch 按
   `full_wq[r,k]`（rank + 层 k 在 axis 1）索引权重，但 `StackedDeviceTensor.__getitem__`
   (`pypto/python/pypto/runtime/device_tensor.py:261`) 只支持 leading（rank）维整 shard 切，不支持 axis-1 partial。
   **修（推荐 (a)）**：增强 `StackedDeviceTensor.__getitem__` 支持 `[r,k]` → 返回
   `DeviceTensor(shard_r.base + k*layer_stride*itemsize, [4096,1024], dtype)`（权重池 per-rank full_wq 是
   [12,4096,1024] 连续，axis-1 slice 是合法 sub-view）。改 device_tensor.py（NFS runtime，纯 Python）+ 重跑
   `_stage_whole_faithful_real_ipc -d 0..7` → 期望 RESULT=REAL_WEIGHT_IPC_RUN_CLEAN（dummy KV → logits 非
   真值，但证 N=1 real-weight import_ipc 全链 dispatch 通 = **Task② device 落地**）。备选 (b) build
   per-(protocol,layer) StackedDeviceTensor + 改 _gen_faithful_real host_orch 只索引 `[r]`（签名变大）。
   run#3 log: `WS/logs_n1/0234_ipc_run3.log`。
