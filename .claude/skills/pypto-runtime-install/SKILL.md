...
```bash
cd "$WS"
# venv：已存在就复用（netboot 持久盘上 .venv311 通常还在，别重建）
[ -d .venv311 ] || python3.11 -m venv .venv311
source .venv311/bin/activate
python -m pip install -U pip

# 依赖：先查再装，已有就跳过——别 clobber 环境里已有的（尤其 CANN 镜像自带的
# torch/numpy 可能是 NPU 定制版，用 cpu 版覆盖会出问题）。
# 0162 stable 参考版本：torch 2.12.1+cpu / numpy 2.4.6 / safetensors 0.8.0
have() { python -c "import $1" 2>/dev/null; }
have torch       || pip install "torch==2.12.1+cpu"
have numpy       || pip install numpy
have safetensors || pip install safetensors
have pytest      || pip install pytest
#   （若严格追求 0162 复现，再核对版本：python -c "import torch,numpy,safetensors as s;
#     print(torch.__version__, numpy.__version__, s.__version__)"；不一致再决定是否 pin）

# pypto / simpler：editable 安装（带 native 扩展）。已装过且代码没变可跳过重装；
# 换了 pin / 改了 native 源码才需要重跑（重跑前按下面「踩坑」清 build 缓存）。
python -c "import pypto" 2>/dev/null   || pip install --no-build-isolation -e "$WS/pypto"
python -c "import simpler" 2>/dev/null || pip install --no-build-isolation -e "$WS/pypto/runtime"
```

> **幂等原则**：本步所有命令都"先检查、缺才装"——`python -c "import X"` 能过就跳过；
> venv / editable 安装同理。目的：在已配好的机器（如 0162）上重跑本 skill 不会破坏现有环境，
> 也不会拿 cpu/通用版覆盖 CANN 镜像的定制包。
...
> - **simpler pin `c7fdc574`（stepfun/develop 2026-07-18 merge）的 python bindings 编不过**
>   ——header/cpp 半成品不一致：① `src/common/hierarchical/orchestrator.cpp:41` 调
>   `get_worker(...)` 但 header 只声明 `get_worker_by_id`（同文件 35/47/53 都是
>   `get_worker_by_id`，41 是笔误）；② `import_ipc` 控制在 cpp 有 3 处定义
>   （WorkerEndpoint/LocalMailboxEndpoint/WorkerThread）但 header 无声明。
>   **修法（选 A，保 N1 patch）**：改 `:41` `get_worker`→`get_worker_by_id` + 补 import_ipc
>   三处 header 声明（补全半成品，零运行时行为变化），然后 commit 到 `csy0225/simpler
>   stepfun/develop` + bump `pypto` submodule pin，让 stable pin 真能编过。
>   **不要退 origin/main `7ccd2a5a`**：它**没有** `SIMPLER_COMM_NO_HCCL`（co-tenancy）与
>   `ENABLE_PEER_ACCESS`（跨卡 IPC）patch → 会回退 Phase 16 / co-tenancy 修复，重撞
>   `507899` / co-tenancy hang。**备选**：钉回 `36957c6b`（N1-STABLE-ENV SSOT，编得过 +
>   全 N1 patch + 跑过 20/20），但仅当 pypto-lib 不依赖 c7fdc574 新增的 C++-native import_ipc
>   （36957c6b 走 pure-Python `CTRL_IMPORT_IPC` facade）时可行。
...
> **agent 判定**：① allreduce max=0 且 ② CI runner PASS（token/argmax 匹配 WHOLE_NETWORK_CI.md /
...
| simpler `c7fdc574` python bindings 编不过 | header/cpp 半成品不一致（orchestrator.cpp:41 `get_worker` 笔误 + import_ipc 缺 header 声明） | 补两处（get_worker_by_id + import_ipc header 声明）保 N1 patch；**别退 origin/main 7ccd2a5a**（缺 no-HCCL/peer-access → 回退 507899/co-tenancy）（Step 4） |
