...
> 本文是 `pypto-whole-net-hang-debug` 的实践案例。主线是 2026-07-10 至
> 2026-07-17 的“单个整网程序运行全部 45 层”方案，并向前追溯
> 2026-06-15 至 2026-07-09 的多卡环境、混合专家层、与 vLLM 同卡运行、
> 跨卡通信和原生 INT8 计算等背景。
>
> 本文不只记录最终把控制信号物理空间扩大到 512B 的修改，而是解释：
> 为什么不同阶段都可能显示同一个外层错误码 `507018`；实际又如何依次排除
> 环境、派发、内存不足、跨层内存复用、具体计算 kernel、数值错误和随机通信
> 卡死等不同问题；以及为什么一些“已经修好”的历史结论会被后续更严格的
> 设备测试和正式准出测试推翻。最终目标是把设备任务日志、具体 kernel、
> 跨设备通信边界、buffer 物理布局、数值精度和完整复现环境串成一条可复核的
> 证据链。
>
> 本文中的“N=1”表示**一个 `@pl.program` 承载完整整网**，不是只有 1 层、
> 1 个 token 或 batch size=1。“stall”表示程序没有继续完成，本文统一称为
> “卡死”或“无 forward progress”。
>
> 本文不是新的执行提示或下一会话任务清单。当前准出事实仍以
> `N1-CANONICAL-TEST.md`、`develop/N1/N1-STABLE-ENV-0162-20260717.md`
> 和最终交接文档为准。
