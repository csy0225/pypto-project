...
OUT=$ROOT/campaign/activation/$VARIANT/bs${BATCH}
CONTAINER="moe-act-${VARIANT//[^A-Za-z0-9]/-}-b${BATCH}"

test -d "$ROOT"
test -d "$SOURCE"
test -d "$SCRIPTS"
test -d "$GOLDEN"
test -x "$SCRIPTS/container_activation_sweep_case.sh"
[[ "$VARIANT" =~ ^[A-Za-z0-9._-]+$ ]]
case "$BATCH" in
  1|2|4|7|8|16) ;;
  *)
...
assert report["workload"]["active_batch"] == int(sys.argv[2])
assert report["workload"]["context_len"] == 65536
assert report["workload"]["blocks_per_sequence"] == 512
PY
  echo "[activation-sweep] verified existing $VARIANT BS$BATCH"
  exit 0
fi
mkdir -p "$OUT"
...
printf '%s\n' "$VARIANT" > "$OUT/variant.txt"
printf '%s\n' "$NONCE" > "$OUT/run_nonce.txt"
sha256sum "$SOURCE/SOURCE_SHA256SUMS" | awk '{print $1}' \
  > "$OUT/source_manifest_sha256.txt"
sha256sum \
  "$SCRIPTS/container_activation_sweep_case.sh" \
  "$SCRIPTS/run_0162_activation_sweep_case.sh" \
...
sudo -n "$NC" run --name "$CONTAINER" --rm --net host --ipc host \
  ...
  --env CASE_VARIANT="$VARIANT" \
  --env RUN_NONCE="$NONCE" \
  --env GOLDEN_DIR=/golden \
  --env MATRIX_IMAGE_REF="$IMG" \
  --env CASE_ITERS="${CASE_ITERS:-12}" \
  ...
  -v "$GOLDEN":/golden:ro \
  -v "$OUT":/out \
  "$IMG" bash /campaign-scripts/container_activation_sweep_case.sh \
...
python3 - "$OUT/runtime/five_layer_moe_report.json" "$OUT/variant.txt" "$BATCH" <<'PY'
import json
import sys

path, variant_path, expected_batch = sys.argv[1:]
report = json.load(open(path, encoding="utf-8"))
expected_variant = open(variant_path, encoding="utf-8").read().strip()
assert report["comparisons"]["hidden_l3"]["exact"] is True
assert report["comparisons"]["hidden_l4"]["exact"] is True
assert report["workload"]["active_batch"] == int(expected_batch)
assert report["workload"]["context_len"] == 65536
assert report["workload"]["blocks_per_sequence"] == 512
assert report["source"]["decode_fwd_sha256"]
assert expected_variant.startswith("act-n")
print("activation correctness=PASS")
PY
for index in 0 1 2 3 4 5 6 7; do
  if fuser "/dev/davinci${index}" >/dev/null 2>&1; then
...
echo MOE_ACTIVATION_SWEEP_0162_CASE=PASS
