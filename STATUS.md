...
> **剩余**：整网精度 vs vLLM —— **vLLM W8A8 oracle 已在 0234 起来（2026-07-12，port 8000，`step3p5`，health=200，greedy chat golden = "嗯，用户问了一个非常基础的问题：…"，KV cache 26.9GiB，enforce_eager TP=8 EP `--quantization ascend`；注意 vLLM 要 clean CANN env——`acl` 在 `/usr/local/Ascend/cann/python/site-packages`，别被 pypto PYTHONPATH shadow）**。**但 device 端 A/B 未通**：N=1 harness 喂 dummy 零输入，缺"tensor-input full-network decode runner + vLLM→whole-net KV bridge + decode-step golden dump"（本分支 `test_vllm_golden_e2e_st.py:101` 自己标注 "decode runner is not wired yet"；这套是 G5b track 的机器，在别分支）。这是个大 port。gate 修复本身 = 完整 merge-sort，与 DeepSeek v4 gate.py 一致（analytically correct）；model-logic torch 精度历史已 0.9995（`pypto_all_layers_detail_compare`，但那是 CPU torch，不测 device kernel）。**下一步 = 把 G5b decode-step-golden 或 live A/B 机器 port 到 N=1 分支**（KV bridge 见 memory `g5b_kv_bridge_not_pure_reshape` / `g5b_import_ipc_facade_missing`）。
>
