...
**以上都只过了 source-overlay 门，没有包含它们的 immutable image。**
...
- bs=1 A/B 检测地板：近期整网 bracket `0.616 / 0.634 ms`（K8 曾拿到 `0.0195 ms` 紧 bracket）。
  **收益低于地板的候选不要单独上 A/B/A**，只会得到"统计不可区分"。
- canonical 验收口径：[`reference/canonical-test.md`](reference/canonical-test.md)。

## 4. Attention 当前判断

- SWA tail-window mask 已改显式 typed INT32 数值区间（`63814d4a`）。
  **source-level N=128 `127/128` PASS，镜像级未重跑** ⇒ 不能宣称 SWA 修复无性能回退。
- Full/SWA 可避免的调度 bubble 已闭环；logical task 按 workload 与 architecture profile 推导，
  **不固定 24 个物理核**。A2A3 blocks-per-task `22/16/22`、reduce fan-in=8 **不是跨架构常量**。
...
- `7928a275` / `cd19fe6b`（active-route scheduling）/ `491267c4`（release harness）均为
  `stepfun/develop@69ad31e4` 的祖先。
...
- canonical structural analyzer 仍 `FAIL_CLOSED`：零本地 routed-token 的 early-dispatch task
  缺 AICore swim record（既有限制，非本轮回归；补它 = §6 第 6 条的 `H5`）。
...
**每次作业前必须重新检查卡占用**（非 root `fuser` 不可信，须 `sudo -n fuser` +
`npu-smi info -t proc-mem` 双查、fail-closed），不能沿用旧 session 的空闲结论。
锁：Claude 用 `0162-cards0-7.lock`（dev0–7），codex 用 `0162-cards8-15.lock`（dev8–15），
`0162-full-machine-perf.lock` **仅 A/B/A 发布门**。诊断可并发；A/B/A 必须持整机锁串行、两半都空。
