...
graph TD
    ...
    subgraph MODEL["pypto-lib （模型 + kernel）"]
        WN["whole_decode_step3p5<br/>单个 @pl.program · 45 层整网"]
    end
...
| **pypto-lib** | 模型 + kernel | step3p5 家族。整网入口 `models/step3p5/decode_fwd.py:whole_decode_step3p5`；attention/MoE/gate/dispatch/combine/expert 组件；`weight_loader.py`；集成代码在 `tools/step3p5/`。 |
...
> 生产整网**唯一入口** = `whole_decode_step3p5`
> （`models/step3p5/decode_fwd.py`）。它把完整 Main 45 层跑在
...
> vLLM）。**无 per-layer production dispatcher、rollback Main 或自定义 Main
> selector**。集成代码在 `pypto-lib/tools/step3p5/`。
