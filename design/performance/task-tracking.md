...
> **🟡 2026-08-11 正在复核的可能混淆（position 效应）—— 两个 ITL 结论暂标「待确认」**：
> 我在 dev0-7 上连续两个 A/B/A 的**中间臂**都慢了几乎相同的量 —— K9 `B_k9 +1.72 ms`、
> K8-selective `B_selective +1.89 ms`，而两者改动完全无关（一个改 kernel 的 Wave3、一个改
> host 的 memset 范围），且两次 A1/A2 spread 都只有 `0.047 / 0.157 ms`。量级巧合到必须先
> 排除「**第 2 个 arm 系统性偏慢**」这一 harness 层混淆，否则两个结论都可能是假象。
> **反面证据（削弱该假设）**：codex 的四臂 campaign 里 position 3 的 `B-double-reset`
> `+2.039 ms` 已被**位置无关的 in-runtime 直接计时** `direct_second_memset 1943 µs` 独立解释
> （残差仅 96 µs），且 position 4 的 `A2` 干净（33.971 vs A1 34.083）⇒ 在 dev8-15 上看不到
> position 惩罚。**正在跑的判定实验**：null control `A1_parent / B_null(与 A 完全相同的
> 源码+runtime) / A2_parent`，若 `B_null` 也 `+1.8 ms` 则 K9 与 K8-selective 的 ITL 结论
> 都必须撤回并改用随机化顺序重测；若 `B_null ≈ A` 则两结论成立。
> **不受影响的结论（in-runtime 直接计时，位置无关）**：K8 的 full-clear 成本
> `~1.94 ms/step`、以及 selective 臂测到的 **`memset_all` 每次 broadcast 固定开销
> `≈342 µs`**（见底部 K8-selective 行）。
