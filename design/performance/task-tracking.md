...
> control-prefix 重排 + 单次 `47,616 B` 清零 ⇒ **`−1.7505 ms/step`（`−5.18%`，66× floor）**，
> 四臂 `hidden_sha256` 全等生产 baseline `567b206b…`、`token=14371`。
...
> [更新日志首行](#更新日志)。
> **状态：未落地**（生产代码改动，待用户授权）。被测的 v1 已被 codex 复核否掉
> （它会打断 five-layer / two-layer / multi-program 等其它 persistent 路径），
> 落地件为硬化后的 **v3**（精确匹配 WholeDecode 才启用、其它 domain 回退全窗清零、
> trace `k8_prefix_applied` 供发布门），v3 确认臂已在跑。
