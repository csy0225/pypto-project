...
| K5 | recursive doubling / recursive halving（变体 A 21 stores·224 KiB / B RD3 3 stores·384 KiB / C RS+AG 6 stores·224 KiB） | **P0** | ⬜ | — | K2a, K6a | **被统一定律提到前列（原 P2 末位）。** 既然 `a≈0`、成本是 per-remote-transaction，则减少交易次数是最大杠杆。变体 C（recursive-halving RS + recursive-doubling AG）= 6 轮 × (1 remote_store + 1 notify) = **12 次交易**（vs 现在 35），且不需 remote_load 与 final copy；列数 RS `2048+1024+512` + AG `512+1024+2048` = **7168** vs 现在 `4096+3584+3584+4096 = 15360`（**流量也更少**，此前记的「224→384 KiB 加重流量」只适用变体 B）。按定律**外推预测（必须实测，不是结果）**：bs=16 `12×0.80 + 0.9357×16×(7168/15360) ≈ 9.6+7.0 = 16.6 µs` → 省 `26.8 µs/call = 1.29 ms`；bs=1 `≈10.0 µs` → 省 `19.0 µs/call`。**即使打五折也越过 14 µs 门，且在 bs=16 也有效 —— 这是 K6a 做不到的。若 K6b 死，K5-C 是 P2 唯一还能独立越门的候选。** 代价不变且必须写明：RS/AG **改变 FP32 求和括号** → 精度准出从 hidden sha256 byte-exact 升级为 live vanilla vLLM W8A8 oracle（N=128 逐 token ≥95%，需整机 + oracle 容器）；另有 3 轮串行 barrier 链风险，`wave1_wait` 方差曾 `0.6 → 371 µs` | 2026-08-10 |
...
| K6b | AR payload 按 **runtime** `active_tokens` 裁剪 | P1 | 🟦 | codex | K6a | **store 侧机制已在类型系统层面确认可行。** codegen probe 的报错本身给出了答案：`Cannot reassign 'remote_tile' with a different type: was pl.Tile[[16, 512], pl.BF16], got pl.Tile[[16, 512], pl.BF16, pl.Mem.Vec, pl.TileView(valid_shape=[active_rows, 512])]`（`0162:/mnt/persist/chensiyu/workspace/p2-k6b-runtime-validshape-20260810/codegen-gate-20260810-235102/host.log`）—— **`valid_shape=[active_rows, 512]` 里 `active_rows` 是 runtime scalar 的名字，被符号化带进了 TileType，而 buffer shape 仍是静态 `[16,512]`（静态分配未被破坏）**。故 `MakeRemoteStoreCodegenPTO` 的 `append_dim(valid_shape[0])` 必走 `else` 分支 → `"?"` + `GetExprAsCode(active_rows)` → TSTORE partition size 变运行时值。剩余仅 SSA 问题（同名变量不能重绑成不同类型，改用 `*_raw`/`*_valid` distinct name）。**门的唯一通过判据不是编译成功，而是生成 MLIR 里该 `pto.partition_view` 的 sizes 不再是常量 16、而是 `?` 维 + 引用 `active_rows` 的运行时 SSA**；若仍是常量 16 则说明 valid_shape 在到达 codegen 前被某个 pass const-fold 了。**背景定性**（claude 读 `pypto/src/backend/common/`）：`pto_ops_shared.cpp:124 GetDimStrings()` 注释「Static dims become numeric strings; **dynamic dims become `?`**」，`GetSizeCodes()` 非常量走 `GetExprAsCode` ⇒ partition_view sizes 支持 runtime。**store 侧是设计好的路径**（partition sizes 取自 tile 的 `valid_shape`，文件头注释 `sizes=<tile.valid_shape padded with leading 1s>`）；**load 侧 extent 与 tile 分配耦合**（sizes 取自 `shape=` 参数，同一 `shape_elems` 也决定输出 tile 类型，`tile_buf = GetCurrentResultTarget()` 需静态分配）→ **7 次 remote_load 需上游给 `remote_load` 加 `valid_shape=` kwarg 解耦**。按 bytes 归因（**估计**，列数 TPUT 4096 / load 3584 / store 3584 / copy 4096 = 15360）：store `23.3% → 3.3 µs/call`（**不需上游**）、TPUT `26.7% → 3.7`（走 `_normalize_intlike`，待确认 `chunk_rows` staging tile 约束）、load `23.3% → 3.3`（需上游）。**store+TPUT ≈ 7.0 µs/call = 0.336 ms，单项越不过门但保序、可进 bundle。** 上卡臂**只校验 row0 exact**、其余行不检查、diag 显式记录，避免被误读成 correctness 回退 | 2026-08-10 |
...
**base 校正后关键路径**：A1/B1/B2/C1/C2/C3/C4/D1/D2/G1/H1/I1/I2/J2/K1/K6a 已 ✅；
historical pull C2 仅作回归基线。当前 performance 看板进行中的是
**B3（KV resident/in-place 的连续多轮 row-diff/liveness 证据）**、**J1（formal DFX /
publication / swimlane 收尾）**、**K6b（`set_validshape` 决定性臂定 runtime 裁剪生死）**
与 **K2b（publisher release fence hoist，需上游 pypto 补丁）**；下一优先是
