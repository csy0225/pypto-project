...
| K6b | AR payload 按 **runtime** `active_tokens` 裁剪 | P1 | 🟦 | codex | K6a | **claude 读 pypto 后端源码定性：runtime extent 是被设计支持的，但 store / load 两侧不对称。** `pto_ops_shared.cpp:124 GetDimStrings()` 注释原文「Static dims become numeric strings; **dynamic dims become `?`**」，`GetSizeCodes()` 非常量走 `GetExprAsCode` ⇒ `pto.partition_view` sizes 可为 runtime 表达式。**store 侧 = 设计好的路径**：`MakeRemoteStoreCodegenPTO`（`pto_ops_distributed.cpp:324`）的 partition sizes **取自 tile 的 `valid_shape`**（`append_dim` 显式含非常量分支，文件头注释 `sizes=<tile.valid_shape padded with leading 1s>`）→ `set_validshape(tile, runtime_rows, cols)` 后 `remote_store`，源 tile 仍静态分配 `[16,512]` 但只搬 `valid_shape[0]` 行，**7 次 remote_store 可 runtime 缩、不需上游**。**load 侧 = extent 与 tile 分配耦合**：partition sizes 取自 `shape=` 参数本身，而同一 `shape_elems` 也决定输出 tile 类型，`tile_buf = GetCurrentResultTarget()` 需静态分配 → **7 次 remote_load 需上游加 `valid_shape=` kwarg 解耦 DMA extent 与 tile 分配**。按 bytes 归因（**估计**，列数 TPUT 4096 / load 3584 / store 3584 / copy 4096 = 15360）：store `23.3% → 3.3 µs/call`、TPUT `26.7% → 3.7`（tensor 级走 `_normalize_intlike`，待确认 staging tile 约束）、load `23.3% → 3.3`（需上游）。**store+TPUT ≈ 7.0 µs/call = 0.336 ms，单项越不过门但保序、可进 bundle。****决定性实验改两步、第一步无卡**：① `compile_gate.sh`（~13.5 s，0 卡）编译最小 probe `set_validshape → remote_store`，grep 生成 MLIR 该 partition_view 的 sizes 是否变 dynamic（`?` + 运行时 SSA）—— 是则机制确认直接进产品实现，否则查 Python→IR 段是否把非常量 expr 带进 TileType 的 valid_shape；② 仅在①通过后上卡量 `3.3 µs` 是否兑现，该臂**只校验 row0 exact**、其余行不检查、diag 显式记录，避免被误读成 correctness 回退 | 2026-08-10 |
