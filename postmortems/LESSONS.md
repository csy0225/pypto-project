...
| 比较两组性能 / 发布一个 env 优化 | **digest、单个 p50、同名源码栈都不等于 matched 运行合同**。先 diff 命令、iters、runner、安全模型、effective env、overlay 与 image Config；matched 回退修复 PASS 不等于刷新历史最优。r9 的 `none/all` 仅一行 env 就差 `5.559 ms` | [18](18-upgrade-itl-fixed-cost-runtime-contract.md)、[`2026-09-02 对账`](../benchmark/2026-09-02-k8-historical-performance-reconciliation.md) |
