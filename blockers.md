...
## 🔴 ACTIVE 2026-07-16 — N1-S-0234：pypto-lib 同步后记录的 whole-net stall，待完整 manifest 复核

**范围必须分开**：

```text
N1-S-0162 = release-qualified
  pypto-lib 0e7a0fdd exact-source / P42 / pull+pull
  20/20 argmax=303

N1-S-0234 = active / root cause unknown
  项目记录称 pypto-lib 三个 release 文件与 0e7a0fdd byte-match 后，
  devices 0..7 fresh canonical 3/3 stall
  完整 pypto/simpler/runtime binary/environment 等价性未验证
  本次审计无法通过 SSH 重新核验该记录
```

0162 的 release gate 已关闭，但该结论不能外推到 0234。也不能把“0234
只拉了 pypto-lib”当成三仓、binary 和环境均一致。本次尝试：

```text
ssh infra@gpu-a910x-0234.host.platform.shaipower.com
```

返回 `Permission denied (publickey,password)`；因此 0234 的 3/3 结果当前
只能标记为**项目既有记录、未由本次 session 独立复核**。

**优先排查**：

1. 核对 pypto-lib、pypto、runtime/simpler 三仓 commit、dirty diff、submodule；
2. 核对 editable/source 路径实际加载文件，以及 runtime `.so` hash/mtime；
3. 核对 CANN、PTOAS、checkpoint、device、ring 环境变量；
4. 若三仓和 binary 对齐后仍 stall，保存同轮 TASK/CLUSTER/COND、
   exact `kernel_config.py`、build hash 和 dmesg delta；
5. 不把 512B signal isolation 当作跨机器充分条件或唯一根因。

**解除条件**：恢复 0234 访问，先生成完整 manifest，再按 canonical 文档
重跑；若仍 stall，定位机器/runtime/environment delta 或新的通信边界。

---

## 🔴 ACTIVE 2026-07-16 — Phase 28 live serving：per-layer KV bridge + 3-way HBM / redundant weights

**边界先声明**：0162 standalone N=1 canonical stall gate 已关闭，不再把
历史 push-mode 随机 stall 作为 0162 active blocker。当前 release 是
`dispatch pull + combine pull`，在 gpu-a910x-0162 上真实 W8A8、真实 KV-IPC、
P42、token 6127 连续 20/20 `argmax=303`；详情见
[`N1-CANONICAL-TEST.md`](N1-CANONICAL-TEST.md)。

该结论不覆盖上面的 N1-S-0234 blocker。**当前 live serving blocker**：

1. **per-layer KV bridge**：whole-net standalone substrate 仍是 45 层共享的
   `k_cache/v_cache`，只覆盖 ctx=1。live multi-token decode 需要从 vLLM paged
   KV pool 导入 per-layer BF16 KV slice，并把 `block_table`、`slot_mapping`、
   `seq_lens` 等 metadata 按 decode step 传入。
2. **3-way HBM / redundant weights**：vLLM W8A8 常驻权重、exporter 的 whole-net
   INT8 IPC 权重、whole-net runtime working set 同时存在时，0162 live 尝试报
   `207001` OOM。已确认不是 standalone stall，也不是单独调小 ring heap 就能解决；
   需要消除 vLLM/exporter 的重复权重，或完成等价的 in-place/shared-weight
   方案。token-exact live A/B 尚未完成。

**解除条件**：完成 per-layer KV model-side 改造与 device 验证；解决重复权重
和 live HBM 预算；再按 Phase 28 的 live A/B 标准做 token-exact 验收。

**历史定位结论降级**：旧文档中将 PUSH/TPUT、某个 stuck kernel 或 signal bit
直接写成唯一硬件根因的结论已撤下。它们可以作为历史定位线索，但不能替代
当前 release 的 A/B 证据。
